...
import { User, Mail, Github, Award, CheckCircle, Clock, AlertTriangle, Shield, LogOut, Search, ArrowRight } from 'lucide-react';
import './App.css';
...
function App() {
	...
	const handleFellowLogin = async (e) => {
		...
		try {
			...
			const match = allFellows.find(f => f.github_username.toLowerCase() === githubInput.trim().toLowerCase());
			...
			if (match) {
				...
				setError(`GitHub username "${githubInput}" is not registered in the fellowship yet.`);
			}
		} catch (err) {
			setError(err.message);
		}
	...
	return (
		<main>
			...
			{error && (
				<div style={{ background: 'var(--error-bg)', border: '1px solid var(--error-border)', color: 'var(--error)', padding: '0.8rem 1.2rem', borderRadius: '8px', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', textAlign: 'left' }}>
					<AlertTriangle size={18} />
					<div style={{ fontSize: '0.9rem' }}><strong>Error:</strong> {error}</div>
				</div>
			)}

			{success && (
				<div style={{ background: 'var(--success-bg)', border: '1px solid var(--success-border)', color: 'var(--success)', padding: '0.8rem 1.2rem', borderRadius: '8px', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', textAlign: 'left' }}>
					<CheckCircle size={18} />
			...
			{view === 'login' && (
				<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem', marginTop: '2rem' }}>
					...
					<div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
						<div>
							<h3 style={{ marginTop: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><User size={20} className="text-gradient" /> Register</h3>
							<p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: '145%' }}>
								Are you an aspiring fellow? Submit your application form to create your challenge profile and track your onboarding tasks.
							</p>
						</div>
						<button onClick={() => setView('register')} className="btn btn-primary" style={{ width: '100%', marginTop: '1.5rem' }}>
					...
					<div className="glass-panel">
						<h3 style={{ marginTop: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Github size={20} className="text-gradient" /> Fellow Access</h3>
						<p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.2rem', lineHeight: '145%' }}>
							Already registered? Enter your GitHub username to review your personal 10-step progress and view your graduation certificate.
						</p>
					...
					{/* Admin Portal Login */}
					<div className="glass-panel">
						<h3 style={{ marginTop: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Shield size={20} className="text-gradient" /> Mentor Portal</h3>
						<p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.2rem', lineHeight: '145%' }}>
							Administrative area for KulKul mentors to verify applicants, update step checklists, and generate graduation certificates.
						</p>
			...
			{view === 'register' && (
				<div className="glass-panel" style={{ maxWidth: '500px', margin: '2rem auto' }}>
					<h2 style={{ marginTop: 0 }} className="text-gradient">Submit Fellowship Application</h2>
					<p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
						Fill in the details below. This will register you in the system, allow mentors to track your steps, and issue certificates.
					</p>
					...
					<form onSubmit={handleRegister}>
						...
						<div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
							<button type="button" onClick={() => setView('login')} className="btn btn-secondary" style={{ flex: 1 }}>
								Back
							</button>
			...
			{view === 'portal' && role === 'fellow' && currentUser && (
				<div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem', marginTop: '1rem' }}>
					<div className="glass-panel" style={{ textAlign: 'left' }}>
						...
						{/* Header Details */}
						<div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', borderBottom: '1px solid var(--border-glass)', paddingBottom: '1.5rem', marginBottom: '1.5rem' }}>
							<div>
								...
								<div style={{ display: 'flex', gap: '1.5rem' }}>
									...
									<span style={{ fontSize: '0.9rem', color: 'var(--primary-light)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
										<Github size={14} /> @{currentUser.github_username}
									</span>
								</div>
							</div>
							<div>
								<div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>APPLICATION STATUS</div>
								<span className={`badge badge-${currentUser.status}`} style={{ padding: '0.4rem 1rem' }}>
									{currentUser.status}
								</span>
							</div>
						</div>
						...
						{/* Track Enrolled */}
						<div style={{ display: 'flex', gap: '1rem', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border-glass)', marginBottom: '2rem' }}>
							<Award className="text-gradient" size={24} />
							<div>
								<div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>ENROLLED FELLOWSHIP TRACK</div>
								<div style={{ fontWeight: 700, color: 'var(--text-title)' }}>{currentUser.track}</div>
							</div>
						</div>

						{/* Read Only Checklist */}
						<h3 style={{ margin: '0 0 0.25rem 0' }}>Your Onboarding Progress</h3>
						<p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1.2rem' }}>
							Mentors check off items as they verify your pull requests and challenges. (Read-only view).
						</p>
							...
							{STEPS.map(step => {
								...
								return (
									<div
										...
										style={{ cursor: 'not-allowed', opacity: isChecked ? 1 : 0.6 }}
									>
										<input
											type="checkbox"
						...
						{currentUser.status === 'graduated' && (
							<div className="certificate-card" style={{ marginTop: '2.5rem' }}>
								...
								<h2 style={{ color: 'hsl(45, 90%, 50%)', margin: '0 0 0.5rem 0', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
								...
								<p style={{ maxWidth: '600px', margin: '0 auto 1.5rem', color: 'hsl(45, 10%, 70%)', fontSize: '0.95rem', lineHeight: '150%' }}>
									For successfully completing the peer-facilitated study tracks, publishing operational serverless logic, and demonstrating AI-native code engineering workflows.
								</p>
				...
				<div className="dashboard-grid">
						...
						<div className="glass-panel">
							...
							<div className="fellows-list">
								...
								{filteredFellows.length === 0 && (
									<p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', padding: '1rem' }}>No matching candidates found.</p>
								)}
							</div>
						</div>
					</div>
					...
					<div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
						{selectedFellow ? (
							<>
								{/* Details Header */}
								<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-glass)', paddingBottom: '1.5rem', textAlign: 'left' }}>
									<div>
										...
										<div style={{ display: 'flex', gap: '1.5rem' }}>
											...
											<a
												...
												style={{ fontSize: '0.9rem', color: 'var(--primary-light)', display: 'flex', alignItems: 'center', gap: '0.3rem', textDecoration: 'none' }}
											>
								...
								<div style={{ display: 'flex', gap: '1rem', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border-glass)', textAlign: 'left' }}>
									...
									<div>
										...
										<div style={{ fontWeight: 700, color: 'var(--text-title)' }}>{selectedFellow.track}</div>
									</div>
								</div>

								{/* Interactive Checklist progress */}
								<div style={{ textAlign: 'left' }}>
									<h3 style={{ margin: '0 0 0.25rem 0' }}>Verify Challenge Progress Checklist</h3>
									<p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '1.25rem' }}>
										Click checkboxes to toggle task completion states in the database.
									</p>
							...
							<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: '300px', color: 'var(--text-muted)' }}>
								...
								<p>Select a fellow from the sidebar list to modify details or toggle checklists.</p>
							</div>
						)}
					</div>
