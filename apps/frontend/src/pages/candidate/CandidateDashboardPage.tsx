...
export const CandidateDashboardPage: React.FC = () => {
  ...
  const [applicationToDelete, setApplicationToDelete] = useState<CandidateApplicationItem | null>(null);

  const deleteApplicationMutation = useMutation({
    mutationFn: async (applicantId: string) => {
      await apiClient.delete(`/candidate/applications/${applicantId}`);
    },
    onSuccess: () => {
      toast.success('Application deleted successfully. You can now apply again.');
      queryClient.invalidateQueries({ queryKey: ['candidate-applications'] });
      setApplicationToDelete(null);
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.error || err?.response?.data?.message || 'Failed to delete application');
    },
  });
  ...
  return (
    ...
    >
      <div className="space-y-8">
                      ...
                      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-6 border-b border-slate-100">
                        ...
                        <div className="flex items-center gap-3 w-full lg:w-auto">
                          ...
                          <button
                            ...
                            className="p-3 rounded-full hover:bg-red-50 text-slate-400 hover:text-red-600 border border-slate-200 transition shrink-0 ml-auto lg:ml-0"
                            title="Delete application data so you can apply again (Testing)"
                          >
        ...
        {/* Modal: Confirm Delete Application Data (For candidate re-applying / testing) */}
        {applicationToDelete && (
            ...
            <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-3.5 pb-4 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    Reset application to apply again
                  </p>
                </div>
              </div>
              ...
              <div className="text-xs text-slate-600 leading-relaxed bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2">
                <p>
                  ...
                  <span className="font-extrabold text-slate-900">{applicationToDelete.program_name}</span>?
                </p>
                <p className="text-slate-500 text-2xs">
                  This permanently deletes your candidate intake record, assessment scores, and AI interview session for this program, enabling you to apply again from scratch for testing.
                </p>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  type="button"
                  disabled={deleteApplicationMutation.isPending}
                  onClick={() => deleteApplicationMutation.mutate(applicationToDelete.applicant_id)}
