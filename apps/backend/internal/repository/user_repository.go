...
func (r *UserRepository) Create(ctx context.Context, email, passwordHash, name, role string, orgID *uuid.UUID) (*model.User, error) {
	...
	query := `
		...
		RETURNING id, organization_id, email, password_hash, name, COALESCE(avatar_url, ''), role, COALESCE(email_verified, true), activation_token, activation_expires_at, password_reset_token, password_reset_expires_at, created_at, updated_at
	`
	var u model.User
	err := r.pool.QueryRow(ctx, query, orgID, email, passwordHash, name, role).Scan(
		&u.ID, &u.OrganizationID, &u.Email, &u.PasswordHash, &u.Name, &u.AvatarURL, &u.Role, &u.EmailVerified, &u.ActivationToken, &u.ActivationExpiresAt, &u.PasswordResetToken, &u.PasswordResetExpiresAt, &u.CreatedAt, &u.UpdatedAt,
	)
	if err != nil {
...
func (r *UserRepository) CreateUnverified(ctx context.Context, email, passwordHash, name, role string, orgID *uuid.UUID, token string, expiresAt time.Time) (*model.User, error) {
	if r.pool == nil {
		r.mu.Lock()
		defer r.mu.Unlock()
		if _, exists := r.memUsers[email]; exists {
	...
	query := `
		INSERT INTO users (organization_id, email, password_hash, name, role, email_verified, activation_token, activation_expires_at, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, false, $6, $7, now(), now())
		RETURNING id, organization_id, email, password_hash, name, COALESCE(avatar_url, ''), role, COALESCE(email_verified, false), activation_token, activation_expires_at, password_reset_token, password_reset_expires_at, created_at, updated_at
	`
	var u model.User
	err := r.pool.QueryRow(ctx, query, orgID, email, passwordHash, name, role, token, expiresAt).Scan(
		&u.ID, &u.OrganizationID, &u.Email, &u.PasswordHash, &u.Name, &u.AvatarURL, &u.Role, &u.EmailVerified, &u.ActivationToken, &u.ActivationExpiresAt, &u.PasswordResetToken, &u.PasswordResetExpiresAt, &u.CreatedAt, &u.UpdatedAt,
	)
	if err != nil {
		return nil, fmt.Errorf("user_repo: create unverified: %w", err)
	}
	return &u, nil
}
...
func (r *UserRepository) GetByEmail(ctx context.Context, email string) (*model.User, error) {
	...
	query := `
		SELECT id, organization_id, email, password_hash, name, COALESCE(avatar_url, ''), role, COALESCE(email_verified, true), activation_token, activation_expires_at, password_reset_token, password_reset_expires_at, created_at, updated_at
		FROM users
		WHERE email = $1
	`
	var u model.User
...
func (r *UserRepository) FindOrCreateByOAuth(ctx context.Context, id OAuthIdentity) (*model.User, error) {
	...
	err = r.pool.QueryRow(ctx, query, orgID, email, name, role).Scan(
		&user.ID, &user.OrganizationID, &user.Email, &user.PasswordHash, &user.Name, &user.AvatarURL, &user.Role, &user.EmailVerified, &user.ActivationToken, &user.ActivationExpiresAt, &user.PasswordResetToken, &user.PasswordResetExpiresAt, &user.CreatedAt, &user.UpdatedAt,
	)
	if err != nil {
...
func (r *UserRepository) GetByActivationToken(ctx context.Context, token string) (*model.User, error) {
	...
	if r.pool == nil {
		...
		defer r.mu.RUnlock()
		for _, u := range r.memUsers {
			if u.ActivationToken != nil && *u.ActivationToken == token {
				return u, nil
			}
		}
		return nil, ErrUserNotFound
	...
	if err != nil {
		return nil, fmt.Errorf("user_repo: get by activation token: %w", err)
	}
	return &u, nil
}
...
func (r *UserRepository) ActivateUser(ctx context.Context, userID uuid.UUID) error {
	...
	query := `
		UPDATE users
		SET email_verified = true, activation_token = NULL, activation_expires_at = NULL, updated_at = now()
		WHERE id = $1
	`
	tag, err := r.pool.Exec(ctx, query, userID)
	if err != nil {
		return fmt.Errorf("user_repo: activate user: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrUserNotFound
	}
...
func (r *UserRepository) SetActivationToken(ctx context.Context, userID uuid.UUID, token string, expiresAt time.Time) error {
	if r.pool == nil {
		r.mu.Lock()
		defer r.mu.Unlock()
		for _, u := range r.memUsers {
			if u.ID == userID {
				tok := token
				exp := expiresAt
				u.ActivationToken = &tok
				u.ActivationExpiresAt = &exp
				u.UpdatedAt = time.Now()
				return nil
			}
		}
	...
	query := `
		UPDATE users
		SET activation_token = $2, activation_expires_at = $3, updated_at = now()
		WHERE id = $1
	`
	tag, err := r.pool.Exec(ctx, query, userID, token, expiresAt)
	if err != nil {
		return fmt.Errorf("user_repo: set activation token: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrUserNotFound
	}
...
func (r *UserRepository) SetPassword(ctx context.Context, userID uuid.UUID, passwordHash string) error {
	...
	if err != nil {
		return fmt.Errorf("user_repo: set password: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrUserNotFound
	}
...
func (r *UserRepository) SetPasswordResetToken(ctx context.Context, userID uuid.UUID, token string, expiresAt time.Time) error {
	...
	tag, err := r.pool.Exec(ctx, query, userID, token, expiresAt)
	if err != nil {
		return fmt.Errorf("user_repo: set password reset token: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrUserNotFound
	}
...
func (r *UserRepository) GetByPasswordResetToken(ctx context.Context, token string) (*model.User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("user_repo: get by password reset token: %w", err)
	}
	return &u, nil
}
...
func (r *UserRepository) ResetPassword(ctx context.Context, userID uuid.UUID, passwordHash string) error {
	...
	query := `
		UPDATE users
		SET password_hash = $2, password_reset_token = NULL, password_reset_expires_at = NULL, email_verified = true, updated_at = now()
		WHERE id = $1
	`
	tag, err := r.pool.Exec(ctx, query, userID, passwordHash)
	if err != nil {
		return fmt.Errorf("user_repo: reset password: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return ErrUserNotFound
	}
