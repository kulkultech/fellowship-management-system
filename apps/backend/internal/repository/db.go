...
func AutoMigrateAndSeed(ctx context.Context, pool *pgxpool.Pool, logger *slog.Logger) error {
	...
	seedOrgQuery := `
		...
		ON CONFLICT (slug) DO UPDATE SET name = 'Acme Academy', contact_email = 'contact@rsa.org', logo_url = '', updated_at = now()
		RETURNING id::text
	`
	...
	`
	var litOrgID string
	if err := pool.QueryRow(ctx, seedLITOrgQuery).Scan(&litOrgID); err != nil {
		_ = pool.QueryRow(ctx, "SELECT id::text FROM organizations WHERE slug = 'ladies-in-tech' LIMIT 1").Scan(&litOrgID)
	}
	...
	// Clean up only the old obsolete lit-sda program
	_, _ = pool.Exec(ctx, "DELETE FROM programs WHERE slug = 'lit-sda'")

	// Seed LIT Scholarship 2026 program & MCQ Question Banks into PostgreSQL (assigned to Ladies in Tech Network)
	targetLITOrgID := litOrgID
	if targetLITOrgID == "" {
		targetLITOrgID = rsaOrgID
	}
	if err := SeedLITAssessmentPrograms(ctx, pool, targetLITOrgID, logger); err != nil {
		logger.Warn("automigrate: seed lit scholarship program error", slog.Any("error", err))
	}
