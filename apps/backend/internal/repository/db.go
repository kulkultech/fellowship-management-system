...
func AutoMigrateAndSeed(ctx context.Context, pool *pgxpool.Pool, logger *slog.Logger) error {
	...
	ALTER TABLE program_assignments ADD COLUMN IF NOT EXISTS target_applicant_ids JSONB NOT NULL DEFAULT '[]'::jsonb;
	CREATE INDEX IF NOT EXISTS idx_program_assignments_target_applicants ON program_assignments USING gin (target_applicant_ids);
